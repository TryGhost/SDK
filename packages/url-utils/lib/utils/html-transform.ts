...
function extractStyleUrls(style = ''): string[] {
    const urls: string[] = [];
    const regex = /url\(['|"]([^)]+)['|"]\)/g;
    let match;
...
type TransformFunction = (url: string, siteUrl: string, itemPath: string | null, options: HtmlTransformOptions) => string;
...
function htmlTransform(html = '', siteUrl: string, transformFunction: TransformFunction, itemPath: string | null, _options?: HtmlTransformOptions): string {
    ...
    function addReplacement(replacement: Replacement) {
    ...
    ['href', 'src', 'srcset', 'style'].forEach((attributeName) => {
        htmlContent('[' + attributeName + ']').each((ix: number, el: any) => {
